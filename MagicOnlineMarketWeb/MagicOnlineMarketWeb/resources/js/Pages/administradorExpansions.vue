...
const abrirModalMod = (expansio) =>{
    cerrarModal();
    showModalModificacio.value=true;
    formExpansio.nom=expansio.nom;
    formExpansio.id=expansio.idExpansio;
    formExpansio.dataSortida = formatejarData(expansio.dataSortida);

}

const formatejarData = (dataHora) => {
    const parts = dataHora.split(' ');
    return parts[0];
}


const modificarExpansio = () => {
    formExpansio.post('editarExpansio');
    finModificacio();
...
const eliminarExpansio =  () => {
    formExpansio.delete(`eliminarExpansio`);
    finEliminacio();
...
const crearExpansio=()=> {
    ...
    formExpansio.post('crearExpansio');
    confirmacionCreacio();
}
    ...
    <AuthenticatedLayout>
        ...
        <div class="d-flex justify-content-center m-3 ">
            <table class="table table-striped my-table w-50 ">
                ...
                <tbody>
                <tr v-for="expansio in expansions" :key="expansio.id">
                    <td>{{expansio.nom}}</td>
                    <td>{{formatejarData(expansio.dataSortida)}}</td>
                    <td>
                        <button  class="btn btn-success rounded-pill"
                                 @click="abrirModalMod(expansio)">Modificar</button>

                    </td>
                    <td>
                        <button  class="btn btn-danger rounded-pill"
                                 @click="abrirModalConfirmacion(expansio.idExpansio)">Eliminar</button>
            ...
            <Modal :show="showModal" maxWidth="2xl" closeable @close="cerrarModal" >
                <div class="modal-content w-100">
                    <div class="d-flex justify-content-center m-3 ">
                        <p>¿Estas segur de que vols eliminar aquesta expansió?</p>
                    </div>
                    <div class="d-flex justify-content-center m-3 ">
                        <button type="button" class="btn btn-success mr-5"
                                @click="eliminarExpansio">Sí</button>
                ...
                <div class="d-flex justify-content-center ">
                    <div class="w-50 rounded my-table"  style="padding: 20px; margin: 20px;">
                        <VForm v-slot="{ errors }" @submit="crearExpansio" class="w-100 rounded">
                            ...
                            <div class="d-flex flex-column align-items-center m-4 p-1">
                                <InputLabel for="data" value="Data Sortida:" />
                                <VField id="data" name="data" type="date" v-model="formExpansio.dataSortida" rules="required" class="mt-1 block w-full" style="color: black;" />
                                <ErrorMessage name="data" style="color: red; font-weight: bold;"/>
                            </div>
                            <div class="d-flex justify-content-center m-3 ">
                                <button class="btn btn-success mr-5" :class="{ 'opacity-25': formExpansio.processing }" :disabled="Object.keys(errors).length > 0">Crear</button>
                ...
                <div class="d-flex justify-content-center ">
                    <div class="w-50 rounded my-table"  style="padding: 20px; margin: 20px;">
                        <VForm v-slot="{ errors }" @submit="modificarExpansio" class="w-100 rounded">
                            ...
                            <div class="d-flex justify-content-center m-3 ">
                                <button class="btn btn-success mr-5" :class="{ 'opacity-25': formExpansio.processing }" :disabled="Object.keys(errors).length > 0">Modificar</button>
                                <button type="button" class="btn btn-danger ml-5"
        ...
        <div class="d-flex justify-content-center m-3 ">
            <button  class="btn btn-success"
                     @click="abrirModalCreacio()">Crear Expansió</button>
        </div>
    </AuthenticatedLayout>
</template>
